...
@Composable
fun DetailWeatherBottomSheet(
    weatherPrompt: PromptStateVo,
    onDismiss: () -> Unit
) {
    AtmosBottomSheet(
        title = stringResource(R.string.now_weather_summary_title),
        onDismiss = onDismiss
    ) {
        DetailWeatherContent(
...
@Composable
private fun DetailWeatherContent(
    weatherPrompt: PromptStateVo
) {
    Column(
        modifier = Modifier
        ...
        if (weatherPrompt.loadingAiPrompt) {
            AtmosAnimation(type = AnimationsType.LoadingAi, size = 120.dp)
        } else {
            if (weatherPrompt.promptResult.isNullOrBlank()) {
                Column(
                    verticalArrangement = Arrangement.Center,
                    horizontalAlignment = Alignment.CenterHorizontally
                ) {
                ...
                AtmosText(
                    ...
                    text = weatherPrompt.promptResult,
                    type = TextType.Description
                )
            }
        }
